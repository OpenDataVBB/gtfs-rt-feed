...
const createMergeVdvFahrtWithRefAusSollFahrtAndAusIstFahrts = async (cfg) => {
	...
	const _storeVdvFahrt = async (fahrtId, vdvFahrt, kind) => {
		...
		if (kind === KIND_ISTFAHRT && !isKomplettfahrt) {
			fieldsArgs = halts.flatMap((istHalt, i) => {
	...
	const storeRefAusSollFahrt = async (fahrtId, sollFahrt) => {
		await _storeVdvFahrt(fahrtId, sollFahrt, KIND_SOLLFAHRT)
	}
	const storeAusIstFahrt = async (fahrtId, istFahrt) => {
		await _storeVdvFahrt(fahrtId, istFahrt, KIND_ISTFAHRT)
	}
	...
	const readEquivalentVdvFahrts = async (fahrtId, vdvFahrt) => {
		const storageKey = STORAGE_KEY_PREFIX + fahrtId
	...
	const mergeVdvFahrtWithEquivalentRefAusSollFahrtAndAusIstFahrts = async (fahrtId, vdvFahrt) => {
		const {
			refAusSollFahrt,
			komplettfahrtAusIstFahrt,
			partialAusIstFahrts,
		} = await readEquivalentVdvFahrts(fahrtId, vdvFahrt)
