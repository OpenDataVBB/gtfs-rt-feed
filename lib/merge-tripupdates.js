...
const stopTimeUpdatesAreEquivalent = (sTU1, opt = {}) => (sTU2) => {
	const {
		...
		stusStopIdsAreEqual: (stuA, stuB) => stuA.stop_id === stuB.stop_id,
	...
	const equalStopIds = stusStopIdsAreEqual(sTU1, sTU2)
...
const mergeStopTimeUpdates = (schedSTU, rtSTU) => {
	const sTU = {
		...mergeButPreferNonNull(schedSTU, rtSTU),
...
const markStopTimeUpdateAsSkipped = (schedSTU) => {
	return {
		...schedSTU,
...
const combineStopTimeUpdates = (schedSTUs, rtSTUs, opt = {}) => {
	...
	while (true) {
		...
		if (rtSTUsI >= rtSTUs.length) {
			let remainingSchedSTUs = schedSTUs.slice(schedSTUsI)
			// If the realtime STUs are exhaustive, and if these schedule STUs have no match, then they must be cancelled a.k.a. skipped.
			if (rtSTUsAreExhaustive) {
				remainingSchedSTUs = remainingSchedSTUs.map(markStopTimeUpdateAsSkipped)
			}
		...
		let schedSTU = schedSTUs[schedSTUsI]
		if (!rtSTU && !schedSTU) {
			break // done!
		}
		...
		if (iMatchingRtSTU < 0) {
			...
			if (rtSTUsAreExhaustive) {
				schedSTU = markStopTimeUpdateAsSkipped(schedSTU)
			}
			merged.push(schedSTU)
			schedSTUsI++
			continue
...
deepStrictEqual(
	combineStopTimeUpdates(
		...
		{
			stusStopIdsAreEqual: (stuA, stuB) => stuA.stop_id.trim() === stuB.stop_id.trim(),
		},
	),
	[
		{
...
const kMyStationId = Symbol('custom station ID, for testing purposes')
deepStrictEqual(
	combineStopTimeUpdates(
		[ // schedSTUs
			{
		...
		{
			stusStopIdsAreEqual: (stuA, stuB) => stuA[kMyStationId] === stuB[kMyStationId],
		},
	),
	[
		{ // merged using `kMyStationId`
			stop_id: 'a1',
			arrival: {time: 2100, delay: 100},
			departure: {time: 3000},
		},
...
const mergeTripUpdates = (schedTU, rtTU, opt = {}) => {
	const {
		...
		rtTripUpdateIsExhaustive,
	} = {
		timeAllowFuzzyMatching: false,
		rtTripUpdateIsExhaustive: false,
		...opt,
	}
	...
	const tU = mergeButPreferNonNull(schedTU, rtTU)
	tU.stop_time_update = combineStopTimeUpdates(
		schedTU.stop_time_update,
		rtTU.stop_time_update,
		{
			...opt,
			rtSTUsAreExhaustive: rtTripUpdateIsExhaustive,
		}
	)
	...
	if (kFahrtID in schedTU) {
		Object.defineProperty(tU, kFahrtID, {
			value: schedTU[kFahrtID],
		})
	}
	if (kUmlaufID in schedTU) {
		Object.defineProperty(tU, kUmlaufID, {
			value: schedTU[kUmlaufID],
		})
	}
	return tU
}
