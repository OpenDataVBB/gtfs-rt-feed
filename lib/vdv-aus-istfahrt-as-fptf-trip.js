...
// some LinienIDs are prefixed with the operator short name, so we strip it
// todo: write tests
const lineIdFromIstFahrt = (istFahrt, operator) => {
	const linienId = istFahrt.LinienID
	if (!linienId) {
		return null
	}

	{
		const op = operator?.ausFahrtBezeichnerSuffix
		if (op && linienId.length > op.length && linienId.slice(0, op.length) === op) {
			return linienId.slice(op.length)
		}
	}
...
const operatorFromVdvAusIstFahrt = (istFahrt) => {
	...
	if (istFahrt.LinienID && dlidRegex.test(istFahrt.LinienID)) {
		const [_, opShortName] = dlidRegex.exec(istFahrt.LinienID)
		const op = OPERATORS.find((op) => {
...
const createFormatVdvAusIstFahrtAsFptfTrip = async (cfg) => {
	...
	const formatVdvAusIstFahrtAsFptfTrip = async (istFahrt) => {
		...
		return {
			...
			line: {
				id: istFahrt.LinienID ? lineIdFromIstFahrt(istFahrt, operator) : null,
				...
				// note: this is in .line, but it should be in .
				// https://github.com/derhuerst/find-hafas-data-in-another-hafas/blob/4.4.0/merge-leg.js
