...
const pickStopTimeUpdatesForMatching = async (cfg, tripUpdate) => {
	...
	// pick most important stop among window of STUs
	{
		...
		// const i0Window = sTUs.slice(i0WindowStartI, i0WindowEndI + 1)
		// console.error({i0WindowStartI, i0WindowEndI, i0Window}) // todo: remove

		const [iHighest] = await findSTUWithHeighestStationWeight(i0WindowStartI, i0WindowEndI)
		i0 = iHighest
	}
	...
	// pick most important stop among window of STUs, just like with `i0`
	{
		...
		// const iNWindow = sTUs.slice(iNWindowStartI, iNWindowEndI + 1)
		// console.error({iNWindowStartI, iNWindowEndI, iNWindow}) // todo: remove

		const [iHighest] = await findSTUWithHeighestStationWeight(iNWindowStartI, iNWindowEndI)
		iN = iHighest
	}
...
const createMatchGtfsRtTripUpdateWithScheduleStopTimes = async (cfg, opt = {}) => {
	...
	const matchGtfsRtTripUpdateWithScheduleStopTimes = async (tripUpdate) => {
		...
		const stopTimes = [
			[
				'st' + iSt0, // alias
				formatStopTimeUpdateAsScheduleStopTime(tripUpdate.stop_time_update[iSt0]),
				{
					stopIdAllowFuzzyIfoptMatching: true,
					timeAllowFuzzyMatching: true,
				},
			...
			[
				'st' + iStN, // alias
				formatStopTimeUpdateAsScheduleStopTime(tripUpdate.stop_time_update[iStN]),
				{
					stopIdAllowFuzzyIfoptMatching: true,
					timeAllowFuzzyMatching: true,
				},
