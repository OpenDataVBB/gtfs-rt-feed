...
const kStationId = Symbol(`GTFS Schedule stop's parent station ID`)
...
// The matching operations are slow, so we need a rather large pool.
const DEFAULT_PG_POOL_SIZE = parseInt(process.env.MATCHING_PG_POOL_SIZE || '30')

const DEFAULT_APPLICATION_NAME = process.env.PGAPPNAME || `${pkg.name}-match-with-schedule-trip`
...
const formatScheduleStopTimeAsStopTimeUpdate = (st) => {
	...
	Object.defineProperty(sTU, kStationId, {value: st.station_id})
	return sTU
}
...
const stusHaveSameIfoptStationId = (stuA, stuB) => {
	const {
		stop_id: stopIdA,
	} = stuA
	const stationIdA = stuA[kStationId] || NaN // `NaN === NaN` is false
	const {
		stop_id: stopIdB,
	} = stuB
	const stationIdB = stuB[kStationId] || NaN
	...
	// Note: With IFOPT-style IDs, we only compare their 3rd level (station) here! We assume that there are no 2 stop IDs with equal 3rd level but different 1st/2nd level *within one trip/TripUpdate*.
	// This implementation mirrors the stop_id/station_id `LIKE` filters in the SQL query in lib/query-schedule-stop-times.js.
	// todo: get rid of this horrible workaround, or at least make it opt-in!
	const _ifoptLvl3 = id => /^[a-z]{2}:\w+:(\w+)(::?\w+)*$/ig.exec(id)?.[1]
	const _stopIdIfopt3A = _ifoptLvl3(stopIdA) ?? NaN // `NaN === NaN` is false
	const _stopIdIfopt3B = _ifoptLvl3(stopIdB) ?? NaN
	const _stationIdIfopt3A = stationIdA ? _ifoptLvl3(stationIdA) : NaN
	const _stationIdIfopt3B = stationIdB ? _ifoptLvl3(stationIdB) : NaN

	// match "horizontally": stop-stop or station-station
	if (
		// both IDs are IFOPT-style
		_stopIdIfopt3A === _stopIdIfopt3B
		|| _stationIdIfopt3A === _stationIdIfopt3B
		// one ID is IFOPT-style, the other isn't
		|| stopIdA === _stopIdIfopt3B
		|| stopIdB === _stopIdIfopt3A
		|| stationIdA === _stationIdIfopt3B
		|| stationIdB === _stationIdIfopt3A
	) {
		return true
	}

	// match across stop/station hierarchy
	return (
		// both IDs are IFOPT-style
		_stopIdIfopt3A === _stationIdIfopt3B
		|| _stopIdIfopt3B === _stationIdIfopt3A
		// one ID is IFOPT-style, the other isn't
		|| stopIdA === _stationIdIfopt3B
		|| stopIdB === _stationIdIfopt3A
		|| stationIdA === _stopIdIfopt3B
		|| stationIdB === _stopIdIfopt3A
	)
}
{ // based on .stop_id
	{ // both IDs are IFOPT-style
		// match on IFOPT levels 1-3
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:12:23:34'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:12:23'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23'}, {stop_id: 'de:12:23:34'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:12:23:45'}), true)
		// mismatch on IFOPT 1st level, ignored
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'pl:12:23:34'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'pl:12:23'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23'}, {stop_id: 'pl:12:23:34'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'pl:12:23:45'}), true)
		// mismatch on IFOPT 2nd level, ignored
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:00:23:34'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:00:23'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23'}, {stop_id: 'de:00:23:34'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:00:23:45'}), true)
		// mismatch on IFOPT 3rd level
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:12:00:34'}), false)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:12:00'}), false)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23'}, {stop_id: 'de:12:00:34'}), false)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: 'de:12:00:45'}), false)
	}
	{ // one ID is IFOPT-style, the other isn't
		// match on IFOPT level 3
		strictEqual(stusHaveSameIfoptStationId({stop_id: '23'}, {stop_id: 'de:12:23:34'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: '23'}, {stop_id: 'de:12:23'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23'}, {stop_id: '23'}), true)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: '23'}), true)
		// mismatch on IFOPT 3rd level
		strictEqual(stusHaveSameIfoptStationId({stop_id: '23'}, {stop_id: 'de:12:00:34'}), false)
		strictEqual(stusHaveSameIfoptStationId({stop_id: '23'}, {stop_id: 'de:12:00'}), false)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23'}, {stop_id: '00'}), false)
		strictEqual(stusHaveSameIfoptStationId({stop_id: 'de:12:23:34'}, {stop_id: '00'}), false)
	}
}
{ // based on `kStationId`
	{ // both IDs are IFOPT-style
		strictEqual(stusHaveSameIfoptStationId(
			{stop_id: 'foo', [kStationId]: 'de:12:23'},
			{stop_id: 'bar', [kStationId]: 'de:12:23'}
		), true)
		strictEqual(stusHaveSameIfoptStationId(
			{stop_id: 'foo', [kStationId]: 'de:12:23'},
			{stop_id: 'bar', [kStationId]: 'de:12:23:34'}
		), true)
		strictEqual(stusHaveSameIfoptStationId(
			{stop_id: 'foo', [kStationId]: 'de:12:23'},
			{stop_id: 'bar', [kStationId]: 'de:00:23'}
		), true)
		strictEqual(stusHaveSameIfoptStationId(
			{stop_id: 'foo', [kStationId]: 'de:12:23'},
			{stop_id: 'bar', [kStationId]: 'de:12:00'}
		), false)
	}
	{ // one ID is IFOPT-style, the other isn't
		strictEqual(stusHaveSameIfoptStationId(
...
const createMatchGtfsRtTripUpdateWithScheduleStopTimes = async (cfg, opt = {}) => {
	...
	const gtfsDataImported = new Gauge({
		name: 'gtfs_data_imported_timestamp_seconds',
		help: 'when GTFS data has been imported into the currently used database',
		registers: [register],
	})

	const pg = await connectToPostgres({
		max: DEFAULT_PG_POOL_SIZE,
		application_name: DEFAULT_APPLICATION_NAME,
		...pgOpts,
	})

	// read Schedule feed digest from DB
	const {
		rows: [{
			feed_digest: scheduleFeedDigest,
		}],
	} = await pg.query('SELECT feed_digest FROM _feed_digest')
	...
	try {
		...
		} = await pg.query('SELECT feed_version FROM feed_info LIMIT 1')
		scheduleFeedVersion = feed_version
	} catch (err) {
		if (err?.code === PG_UNDEFINED_TABLE) {
			logger.debug('GTFS import does not have a feed_info table')
		} else if (err?.code === PG_UNDEFINED_COLUMN) {
			logger.debug(`GTFS import's feed_info table does not have a feed_version column`)
		} else {
			throw err
		}
	}

	// expose gtfs-via-postgres' gtfs_data_imported_at() as metric
	{
		const {
			rows: [{
				gtfs_data_imported_at,
			}],
		} = await pg.query('SELECT gtfs_data_imported_at()')
		if (gtfs_data_imported_at !== null) {
			gtfsDataImported.set(gtfs_data_imported_at / 1000)
		} else {
			logger.warn('db: gtfs_data_imported_at() has no value')
		}
	}
	...
	const cache = await createCache({
		...
		prefix: 'match:' + scheduleFeedDigest.slice(0, 4),
	...
	const matchGtfsRtTripUpdateWithScheduleStopTimes = async (tripUpdate, opt = {}) => {
		const {
			tripUpdateIsExhaustive,
			tripUpdateIncludesServiceStops,
		} = {
			tripUpdateIsExhaustive: false,
			tripUpdateIncludesServiceStops: true,
			...opt,
		}
		...
		const [
			...
			logger,
			queryStationWeight,
			// 3 is too small for VBB buses, with many not-so-important stops among a single important one
			windowSize: 5, // todo: make customisable
		...
		let isCached = false
		{
			const cacheId = sha256AsHex([
				scheduleFeedDigest,
				// todo: stable stringifier?
				JSON.stringify(stopTimes),
			].join(':'))
			...
			if (matchedStopTimesFromCache !== null) {
				...
				const {
					...
					name: `stop_times_${createHash('sha1').update(query).digest('hex')}`,
					text: query,
					values: params,
				})
		...
		const mergedTripUpdate = mergeTripUpdates(scheduleTripUpdate, tripUpdate, {
			stusStopIdsAreEqual: stusHaveSameIfoptStationId,
			timeAllowFuzzyMatching: true,
			rtTripUpdateIsExhaustive: tripUpdateIsExhaustive,
			rtTripUpdateIncludesServiceStops: tripUpdateIncludesServiceStops,
		})

		// todo: add flag `stopIdsAreIfoptFormatted` and only then do this
		if (tripUpdateIsExhaustive && !tripUpdateIncludesServiceStops) {
			// see also the test case in test/matching-vbb-m29.js
			// see also https://github.com/OpenDataVBB/gtfs-rt-infrastructure/issues/8#issuecomment-5141769606
			const sTUs = mergedTripUpdate.stop_time_update
			const lastSTU = sTUs[sTUs.length - 1]
			const secondLastSTU = sTUs[sTUs.length - 2]
			if (
				lastSTU.schedule_relationship === STOPTIMEUPDATE_SCHEDULE_RELATIONSHIP_SKIPPED
				&& secondLastSTU?.schedule_relationship === STOPTIMEUPDATE_SCHEDULE_RELATIONSHIP_SCHEDULED
				&& stusHaveSameIfoptStationId(lastSTU, secondLastSTU)
			) {
				// remove schedule_relationship=SKIPPED from "service stops"
				lastSTU.schedule_relationship = STOPTIMEUPDATE_SCHEDULE_RELATIONSHIP_NO_DATA
			}
		}
