...
const kUmlaufID = Symbol('AUS IstFahrt.UmlaufID')
...
const formatVdvAusIstHaltAsGtfsRtStopTimeUpdate = (istHalt) => {
	...
	const stopTimeUpdate = {
		...
		// todo: expose istHalt.Zusatzhalt using TripModifications? (https://gtfs.org/documentation/realtime/feed-entities/trip-modifications/)
		...
		schedule_relationship: istHalt.FaelltAus === 'true'
			? STOPTIMEUPDATE_SCHEDULE_RELATIONSHIP_SKIPPED
			: null,
	}
...
const createFormatVdvAusIstFahrtAsGtfsRtTripUpdate = async (cfg) => {
	...
	const formatVdvAusIstFahrtAsGtfsRtTripUpdate = (istFahrt) => {
		// added by vdv-453-nats-adapter
		// see also https://github.com/OpenDataVBB/vdv-453-nats-adapter/blob/76d7b8c3ece9bdb979df4d4b294e853575f7db21/index.js#L374
		// see also https://github.com/OpenDataVBB/vdv-453-client/blob/4eb1a181ff26267d07fbeca6c9b8f0fa4cb4d3b6/lib/parse-aus-istfahrt.js#L61
		const {
			['$BestaetigungZst']: bestaetigungZst,
		} = istFahrt
		...
		const tripUpdate = {
			// prefer IstFahrt.Zst (indicating the *prognosis'* timestamp) instead of Bestaetigung.Zst (indicating the VDV-454 request's timestamp)
			timestamp: (istFahrt.Zst ?? bestaetigungZst)
				? unixTimestampFromIso8601(istFahrt.Zst ?? bestaetigungZst)
				: null,
			...
			trip: {
				...
				// – https://resources.transitapp.com/article/462-trip-updates
				// todo: with an added trip, what if there is no schedule equivalent?
				schedule_relationship: istFahrt.FaelltAus === 'true'
					? SCHEDULE_RELATIONSHIP_CANCELED
					: (istFahrt.Zusatzfahrt === 'true'
						// todo: adapt to https://github.com/google/transit/pull/504 once it is resolved
						? SCHEDULE_RELATIONSHIP_ADDED
						: null
					),
			},
