...
const VDV_FAHRTS_INCLUDE_SERVICE_STOPS = process.env.VDV_FAHRTS_INCLUDE_SERVICE_STOPS
	? JSON.parse(process.env.VDV_FAHRTS_INCLUDE_SERVICE_STOPS)
	: true
strictEqual(typeof VDV_FAHRTS_INCLUDE_SERVICE_STOPS, 'boolean')
...
const createMatchWithGtfs = async (cfg) => {
	...
	const matchVdvAusIstFahrtWithGtfs = async (vdvAusIstFahrt) => {
		// todo: ignore IstFahrts without any realtime data
		// todo: handle .PrognoseMoeglich
		const isKomplettfahrt = vdvAusIstFahrt.Komplettfahrt === 'true'
		...
		const {
			...
			tripUpdateIsExhaustive: isKomplettfahrt,
			tripUpdateIncludesServiceStops: VDV_FAHRTS_INCLUDE_SERVICE_STOPS,
		})
