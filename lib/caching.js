...
const createCache = async (opt = {}) => {
	...
	const prefix = PREFIX + (_prefix ? _prefix + ':' : '')
	...
	const getMany = async (_prefix, itemsPerScan = 100, maxScans = 5) => {
		if (noCaching) return []
		...
		let keys = []
		let cursor = '0'
		for (let it = 0; it < maxScans; it++) {
			const [newCursor, newKeys] = await redis.scan(
				cursor, // initial value
				'MATCH', prefix + _prefix + '*',
				'COUNT', String(itemsPerScan),
			)
			keys = keys.concat(newKeys)
			cursor = newCursor
		}
		strictEqual(cursor, '0', `SCAN cursor must be 0 after ${maxScans} iterations with up to ${itemsPerScan} items each`)
