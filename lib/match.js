...
import {jsonCodec as natsJson} from './nats.js'
import {
	createMatchWithGtfs,
	closeMatching,
} from './raw-match.js'
...
const runGtfsMatching = (cfg, opt = {}) => {
	...
	const matchTripAndPublish = async (vdvTrip) => {
		try {
			const {
				gtfsTrip,
				isMatched,
				isCached,
				matchingTime,
			} = await matchTripWithGtfs(vdvTrip)

			// todo: refer to SBB/HSL topic structure
			const topic = 'todo' // todo
			natsClient.publish(topic, natsJson.encode(gtfsTrip))
	...
	const processTripMsg = async (msg) => {
		...
		logger.trace('processing trip msg', subject, data.slice(0, 20).toString('utf8'))
		try {
			const trip = msg.json(data)
			await matchTripAndPublish(trip)

			logger.trace(`successfully processed trip msg "${subject}"`)
		} catch (err) {
			// We catch all non-programmer errors in order not to abort the message processing (see below).
			logger.warn({err}, `failure processing trip msg "${subject}": ${err.message || (err + '')}`)
			if (isProgrammerError(err)) {
				throw err
			}
		}
	...
	{
		// todo: subscribe via NATS JetStream durable consumer/subscription? apply limits?
		const tripsSub = natsClient.subscribe(AUS_ISTFAHRT_PREFIX)
