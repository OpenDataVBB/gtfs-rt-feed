...
// The lookup operations are a) fast and b) in-memory-cached, so we can keep the pool small.
const DEFAULT_PG_POOL_SIZE = parseInt(process.env.STATION_WEIGHTS_LOOKUP_PG_POOL_SIZE || '10')

const DEFAULT_APPLICATION_NAME = process.env.PGAPPNAME || `${pkg.name}-station-weight-lookup`
